...
class SubscriptionRepository {
  ...
  async findAllAdmin({ status, search, limit = 50, offset = 0 } = {}) {
    let sql = `
      SELECT s.*, sh.shop_name as "companyName", sh.email as "accountEmail", sh.owner_name as "ownerName"
    ...
    if (status && status !== 'all') {
      sql += ` AND LOWER(s.status) = LOWER($${paramIdx++})`;
      params.push(status);
    }

    if (search && search.trim() !== '') {
      sql += ` AND (LOWER(s.plan_name) LIKE $${paramIdx} OR LOWER(sh.shop_name) LIKE $${paramIdx} OR LOWER(sh.email) LIKE $${paramIdx})`;
