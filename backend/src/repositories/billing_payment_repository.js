...
class BillingPaymentRepository {
  ...
  async findAllAdmin({ status, search, limit = 50, offset = 0 } = {}) {
    let sql = `
      SELECT p.*, sh.shop_name as "companyName", sh.email as "accountEmail", pl.name as "planName"
      FROM payments p
    ...
    if (status && status !== 'all') {
      sql += ` AND LOWER(p.status) = LOWER($${paramIdx++})`;
      params.push(status);
    }

    if (search && search.trim() !== '') {
      sql += ` AND (LOWER(sh.shop_name) LIKE $${paramIdx} OR LOWER(sh.email) LIKE $${paramIdx} OR LOWER(p.transaction_id) LIKE $${paramIdx})`;
      params.push(`%${search.trim().toLowerCase()}%`);
      paramIdx++;
    }

    sql += ` ORDER BY p.paid_at DESC LIMIT $${paramIdx++} OFFSET $${paramIdx++}`;
