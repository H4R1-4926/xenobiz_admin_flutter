...
class AuthService {
  ...
  async login({ emailOrUsername, identifier, password }) {
    ...
    // First check if this is a dedicated admin account login
    ...
    if (!shop) {
      throw { statusCode: 404, message: 'Invalid credentials. Account not found.' };
    }
  ...
  async getCurrentShop(targetId) {
    ...
    if (!shop) {
      throw { statusCode: 404, message: 'Account profile not found.' };
    }
